...
pub fn bbp(n: usize) -> String {
    ...
    let digit = [(4.0, 1), (-2.0, 4), (-1.0, 5), (-1.0, 6)].iter().map(|(num_const, denom_const)| {
        num_const * compute_sum(n, *denom_const) 
    }).sum::<f64>();

    format!("{:x}", ((digit - digit.floor()) * 16.0).floor() as usize)
}
...
fn compute_sum(digit: usize, denom_const: usize) -> f64 {
    ...
    let finite_sum = (0..=digit).fold(0.0, |sum, k| {
        ...
        let term = (mod_pow(16, digit - k, denominator) as f64) / (denominator as f64);
        (sum + term).fract()
    });
